...
# -*- coding: utf-8 -*-
"""
ЭДО ЛДПР - Серверная часть для деплоя на Render
"""
...
def get_db():
    """Получение БД из контекста Flask"""
    if 'db' not in g:
...
# HTML шаблоны (встроенные для простоты)
...
<div class="login-page">
    <div class="login-card">
        ...
        <div class="login-subtitle">
            Система электронного документооборота<br>Либерально-демократической партии России
        </div>
        {% with messages = get_flashed_messages(with_categories=true) %}
        {% if messages %}
        {% for cat, msg in messages %}
        ...
        <form method="POST">
            ...
            <button type="submit" class="btn btn-login">Войти в систему</button>
        </form>
...
<form method="POST" action="/department/create"><div class="modal-body"><div class="mb-3"><label class="form-label">Название отдела</label><input type="text" name="name" class="form-control" required></div></div>
<div class="modal-footer"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Отмена</button><button type="submit" class="btn btn-primary">Сохранить</button></div></form></div></div></div>
{% endblock %}''', departments=departments)
