...
ВСЕ ФУНКЦИИ: распоряжения, отделы, пользователи, статусы, история, фильтры
Адаптировано для Render
"""
...
<head>
    ...
    <style>
        ...
        .login-card { background: white; border-radius: 20px; padding: 40px; max-width: 400px; width: 100%; box-shadow: 0 10px 40px rgba(0,0,0,0.2); }
        .logo { text-align: center; font-size: 28px; font-weight: 900; color: #003399; }
        .btn-login { background: #003399; color: white; width: 100%; padding: 12px; border: none; border-radius: 10px; }
        .btn-login:hover { background: #002266; }
        .test-accounts { font-size: 11px; color: #666; text-align: center; margin-top: 20px; padding-top: 15px; border-top: 1px solid #eee; }
    </style>
</head>
<body>
...
<head>
    ...
    <style>
        ...
        .sidebar .nav-link { color: #555; border-radius: 10px; margin: 5px 8px; padding: 10px 16px; font-weight: 500; transition: all 0.3s; }
        ...
        .avatar { width: 70px; height: 70px; background: #003399; color: white; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 28px; font-weight: bold; margin: 0 auto; }
    </style>
</head>
<body>
    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <div class="col-md-10 p-4">
                ...
                <div class="row mb-4">
                    <div class="col-md-3 mb-3"><div class="card stat-card p-3"><small class="text-muted text-uppercase fw-bold">Мои распоряжения</small><h2 class="fw-bold mb-0 mt-2">{{ stats.total }}</h2></div></div>
                    <div class="col-md-3 mb-3"><div class="card stat-card p-3" style="border-left-color:#f59e0b;"><small class="text-muted text-uppercase fw-bold">На утверждении</small><h2 class="fw-bold mb-0 mt-2">{{ stats.pending }}</h2></div></div>
                    <div class="col-md-3 mb-3"><div class="card stat-card p-3" style="border-left-color:#10b981;"><small class="text-muted text-uppercase fw-bold">Утверждено</small><h2 class="fw-bold mb-0 mt-2">{{ stats.approved }}</h2></div></div>
                    <div class="col-md-3 mb-3"><div class="card stat-card p-3" style="border-left-color:#6366f1;"><small class="text-muted text-uppercase fw-bold">В работе</small><h2 class="fw-bold mb-0 mt-2">{{ stats.in_work }}</h2></div></div>
                </div>
                <div class="card p-4">
                    <div class="d-flex justify-content-between mb-3"><h5 class="fw-bold mb-0"><i class="bi bi-list-ul me-2"></i>Последние распоряжения</h5><a href="/orders" class="btn btn-primary btn-sm">Все распоряжения <i class="bi bi-arrow-right"></i></a></div>
                    {% if orders %}
                    <div class="table-responsive">
                        <table class="table table-hover">
                            <thead class="table-light">
                                <tr><th>№</th><th>Название</th><th>Статус</th><th>Приоритет</th><th>Создано</th></tr>
                            </thead>
                            <tbody>
                                {% for o in orders %}
...
<head>
    ...
    <style>
        ...
        .avatar { width: 70px; height: 70px; background: #003399; color: white; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 28px; margin: 0 auto; }
    </style>
</head>
<body>
    <nav class="navbar navbar-dark">
    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-md-2 sidebar py-4">
                <div class="text-center mb-4">
                    <div class="avatar">{{ current_user.full_name[0] }}</div>
                    <h6 class="mt-2">{{ current_user.full_name }}</h6>
                    <small class="text-muted">{{ current_user.role }}</small>
                </div>
                <nav class="nav flex-column">
                    <a class="nav-link" href="/">Рабочий стол</a>
                    <a class="nav-link active" href="/orders">Распоряжения</a>
                    {% if current_user.role in ['head_department', 'admin'] %}
                    <a class="nav-link" href="/department">Отдел</a>
                    {% endif %}
                    {% if current_user.role == 'admin' %}
                        ...
                        <table class="table table-hover mb-0">
                            ...
                            <tbody>
                                ...
                                <tr onclick="location.href='/orders/{{ o.id }}'" style="cursor:pointer">
                                    ...
                                    <td><span class="badge {% if o.status == 'Утверждено' %}bg-success{% elif o.status == 'На утверждении' %}bg-warning text-dark{% elif o.status == 'Закрыто' %}bg-secondary{% elif o.status == 'Отклонено' %}bg-danger{% else %}bg-primary{% endif %}">{{ o.status }}</span></d>
                                    <td>{{ o.priority }}</d>
                                    <td>{{ o.creator_name }}</d>
                                    <td>{{ o.deadline or '-' }}</d>
                                    <td>{{ o.created_at[:10] }}</d>
                                </tr>
                                {% endfor %}
                            </tbody>
                        </table>
    ...
    <div class="modal fade" id="createModal" tabindex="-1">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header bg-primary text-white"><h5 class="modal-title"><i class="bi bi-file-earmark-plus"></i> Новое распоряжение</h5><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button></div>
                <form method="POST" action="/orders/create">
                    <div class="modal-body">
                        ...
                        <div class="mb-3"><label class="form-label fw-bold">Содержание</label><textarea name="content" class="form-control" rows="8" required></textarea></div>
                    </div>
...
<head>
    ...
    <style>
        .navbar { background: linear-gradient(135deg, #003399, #001a4d); }
        .sidebar { background: white; min-height: calc(100vh - 56px); }
        .sidebar .nav-link { color: #555; border-radius: 10px; margin: 5px 8px; padding: 10px 16px; }
        .sidebar .nav-link:hover { background: #eef; color: #003399; }
        .btn-primary { background: #003399; border: none; }
...
<body>
    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <div class="col-md-10 p-4">
                <a href="/orders" class="btn btn-secondary btn-sm mb-3"><i class="bi bi-arrow-left"></i> Назад</a>
                
                <div class="card p-4 mb-4">
                    <div class="d-flex justify-content-between">
                        <h3 class="fw-bold">{{ order.title }}</h3>
                        <span class="badge {% if order.status == 'Утверждено' %}bg-success{% elif order.status == 'На утверждении' %}bg-warning text-dark{% elif order.status == 'Закрыто' %}bg-secondary{% elif order.status == 'Отклонено' %}bg-danger{% else %}bg-primary{% endif %} fs-6 px-3 py-2">{{ order.status }}</span>
                    </div>
                    <div class="row mt-3">
                        ...
                        <div class="col-md-3"><small class="text-muted">Приоритет</small><br><strong class="{% if order.priority == 'Срочный' %}text-danger{% endif %}">{{ order.priority }}</strong></div>
                        <div class="col-md-3"><small class="text-muted">Создано</small><br><strong>{{ order.created_at[:16] }}</strong></div>
                    </div>
                    <hr>
                ...
                <div class="card p-4">
                    ...
                    <form method="POST" action="/orders/{{ order.id }}/status"><input type="hidden" name="status" value="На доработке"><input type="text" name="comment" class="form-control mb-2" placeholder="Причина доработки"><button class="btn btn-warning w-100"><i class="bi bi-arrow-counterclockwise"></i> На доработку</button></form>
                    {% endif %}
                    ...
                    <div class="small text-muted">
                        <strong>Назначенный отдел:</strong> {% for d in departments %}{% if d.id == order.assigned_department_id %}{{ d.name }}{% endif %}{% endfor %}<br>
                        <strong>Исполнитель:</strong> {% for u in dept_users %}{% if u.uid == order.assigned_executor_id %}{{ u.full_name }}{% endif %}{% endfor %}
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-md-2 sidebar py-4">
                ...
                <nav class="nav flex-column">
                    <a class="nav-link" href="/">Рабочий стол</a>
                    <a class="nav-link" href="/orders">Распоряжения</a>
                    <a class="nav-link active" href="/department">Отдел</a>
                </nav>
            </div>
            <div class="col-md-10 p-4">
                ...
                <div class="card p-4">
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover">
                            ...
                            <tbody>{% for u in users %}<tr><td><strong>{{ u.full_name }}</strong></d><td>{{ u.role }}</d><td>{{ u.email }}</d></tr>{% endfor %}</tbody>
                        </table>
                    </div>
                </div>
            </div>
...
<body>
        ...
        <div class="row">
            ...
            <div class="col-md-10 p-4">
                ...
                <div class="card p-4">
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover">
                            <thead class="table-light"><tr><th>ФИО</th><th>Логин</th><th>Роль</th><th>Email</th></tr></thead>
                            <tbody>{% for u in users %}<tr><td><strong>{{ u.full_name }}</strong></d><td>{{ u.username }}</d><td>{{ u.role }}</d><td>{{ u.email }}</d></tr>{% endfor %}</tbody>
                        </table>
                    </div>
                </div>
            </div>
    ...
    <div class="modal fade" id="addUserModal" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header bg-primary text-white"><h5 class="modal-title">Добавить пользователя</h5><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button></div>
...
def admin_panel():
    ...
    departments = DepartmentModel.get_all()
    return render_template_string(ADMIN_PAGE, users=users, departments=departments)
