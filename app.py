...
# -*- coding: utf-8 -*-
"""
ЭДО ЛДПР - Полная версия для Render
Все функции сохранены, исправлена ошибка TemplateNotFound
"""
...
def seed_database():
    ...
    for d in departments:
        cur.execute(
            "INSERT INTO departments (id, name, head_id) VALUES (%s, %s, %s) ON CONFLICT (id) DO NOTHING",
            d
        )
...
class UserModel:
    ...
    def create(uid, full_name, email, username, password, role='executor', department_id=None):
        ...
        try:
            ...
            return False, f'Ошибка при создании пользователя'
...
class DepartmentModel:
    ...
    def create(dept_id, name, head_id=None):
        ...
        cur = db.cursor()
        try:
            cur.execute("INSERT INTO departments (id, name, head_id) VALUES (%s, %s, %s)", (dept_id, name, head_id))
            db.commit()
            return True
        except Exception:
            return False
...
class OrderModel:
    ...
    def get_by_user(uid, role, department_id=None):
        ...
        if role == 'admin':
            ...
            cur.execute("SELECT * FROM orders WHERE status IN ('Утверждено','В отделе','Назначен исполнитель','В работе','Выполнено','На доработке','Закрыто') ORDER BY created_at DESC")
        else:
            cur.execute("SELECT * FROM orders ORDER BY created_at DESC")
        return cur.fetchall()
...
# ============================================================
# ШАБЛОНЫ (определены в правильном порядке: сначала BASE)
# ============================================================

# БАЗОВЫЙ ШАБЛОН - ДОЛЖЕН БЫТЬ ПЕРВЫМ!
    ...
    <div class="login-page-new">
        <div class="login-card-new">
            <div class="login-logo">
                <div class="login-logo-text">ЭДО ЛДПР</div>
            </div>
            ...
            <div class="mt-3 text-center">
                <small class="text-muted">Тестовые учетные записи:<br>admin/admin123, secretary/sec123, executor/exec123</small>
            </div>
        </div>
    </div>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/js/bootstrap.bundle.min.js"></script>
...
def department():
    if session.get('user_role') == 'admin':
        departments = DepartmentModel.get_all()
        # Упрощенная страница для админа
        dept_list_html = '''
        {% extends "base.html" %}
        ...
        <div class="modal fade" id="addDeptModal" tabindex="-1">
            <div class="modal-dialog"><div class="modal-content">
                ...
                <form method="POST" action="/department/create">
                    <div class="modal-body"><input type="text" name="name" class="form-control" placeholder="Название отдела" required></div>
        ...
        {% endblock %}
        '''
        return render_template_string(dept_list_html, departments=departments)
...
def admin_create_user():
    ...
    if success:
        ...
        flash(error or 'Ошибка при создании пользователя', 'danger')
    return redirect(url_for('admin_panel'))
