...
Исправлена ошибка с декораторами
"""
...
# ============================================================
# ШАБЛОНЫ (СОКРАЩЕННЫЕ ДЛЯ ЭКОНОМИИ МЕСТА)
# ============================================================
...
<body>
    <div class="login-card">
        ...
        <div class="text-center mt-3 small text-muted">admin/admin123 | secretary/sec123 | executor/exec123</div>
    </div>
</body>
</html>'''
...
<html lang="ru">
<head>
    <meta charset="UTF-8"><title>Рабочий стол - ЭДО ЛДПР</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css" rel="stylesheet">
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.3/font/bootstrap-icons.css">
    <style>.navbar{background:#003399}.navbar-brand,.navbar-text{color:white}.sidebar{background:white;min-height:100vh}.sidebar .nav-link{color:#333}.sidebar .nav-link:hover{background:#eef;color:#003399}.card{border-radius:15px}.stat-card{border-left:4px solid #003399;cursor:pointer}</style>
</head>
<body>
...
{% if current_user.role in ['head_department','admin'] %}<a class="nav-link" href="/department">Отдел</a>{% endif %}
{% if current_user.role == 'admin' %}<a class="nav-link" href="/admin">Админ</a><a class="nav-link" href="/admin/stats">Статистика</a>{% endif %}</nav>
</div><div class="col-md-10 p-4">
<h2 class="mb-4">Рабочий стол</h2>
<div class="row mb-4"><div class="col-md-3"><div class="card stat-card p-3" onclick="location.href='/orders'"><small>Мои распоряжения</small><h2>{{ stats.total }}</h2></div></div>
...
<div class="col-md-3"><div class="card stat-card p-3" style="border-left-color:#6366f1" onclick="location.href='/orders?status=В работе'"><small>В работе</small><h2>{{ stats.in_work }}</h2></div></div></div>
<div class="card p-3"><h5>Последние распоряжения</h5><table class="table">{% for o in orders %}<tr onclick="location.href='/orders/{{ o.id }}'" style="cursor:pointer"><td>{{ o.title }}</td><td>{{ o.status }}</td><td>{{ o.created_at[:10] }}</td></tr>{% endfor %}</table></div>
</div></div></div>
</body></html>'''
...
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css" rel="stylesheet">
<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.3/font/bootstrap-icons.css">
<style>.navbar{background:#003399}.sidebar{background:white;min-height:100vh}.sidebar .nav-link{color:#333}.sidebar .nav-link:hover{background:#eef;color:#003399}</style>
</head>
<body>
<nav class="navbar navbar-dark"><div class="container-fluid"><a class="navbar-brand" href="/">ЭДО ЛДПР</a>
<div class="d-flex"><span class="navbar-text me-3">{{ current_user.full_name }}</span><a href="/logout" class="btn btn-outline-light btn-sm">Выход</a></div></div></nav>
...
<div class="text-center mb-4"><div class="bg-primary text-white rounded-circle d-flex align-items-center justify-content-center mx-auto mb-2" style="width:60px;height:60px">{{ current_user.full_name[0] }}</div>
<h6>{{ current_user.full_name }}</h6><small class="text-muted">{{ current_user.role }}</small></div>
<nav class="nav flex-column"><a class="nav-link" href="/">Рабочий стол</a><a class="nav-link active" href="/orders">Распоряжения</a>
{% if current_user.role in ['head_department','admin'] %}<a class="nav-link" href="/department">Отдел</a>{% endif %}
{% if current_user.role == 'admin' %}<a class="nav-link" href="/admin">Админ</a>{% endif %}</nav>
</div><div class="col-md-10 p-4">
<h2>Распоряжения</h2>
{% if current_user.role == 'assistant' %}<button class="btn btn-primary mb-3" data-bs-toggle="modal" data-bs-target="#createModal">+ Создать</button>{% endif %}
<div class="card p-0"><table class="table table-hover"><thead class="table-light"><tr><th>№</th><th>Название</th><th>Статус</th><th>Приоритет</th><th>Автор</th><th>Создано</th></tr></thead>
<tbody>{% for o in orders %}<tr onclick="location.href='/orders/{{ o.id }}'" style="cursor:pointer"><td>#{{ o.id[:8] }}</d><td>{{ o.title }}</d><td><span class="badge bg-primary">{{ o.status }}</span></d><td>{{ o.priority }}</d><td>{{ o.creator_name }}</d><td>{{ o.created_at[:10] }}</d></tr>{% endfor %}</tbody></table></div>
<div class="modal fade" id="createModal" tabindex="-1"><div class="modal-dialog modal-lg"><div class="modal-content"><div class="modal-header bg-primary text-white"><h5>Новое распоряжение</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
<form method="POST" action="/orders/create" enctype="multipart/form-data"><div class="modal-body"><input type="text" name="title" class="form-control mb-2" placeholder="Заголовок" required>
<select name="priority" class="form-select mb-2"><option>Нормальный</option><option>Высокий</option><option>Срочный</option></select>
<input type="date" name="deadline" class="form-control mb-2"><textarea name="content" class="form-control mb-2" rows="5" placeholder="Содержание" required></textarea>
<input type="file" name="order_file" class="form-control"></div>
<div class="modal-footer"><button type="submit" name="is_draft" value="1" class="btn btn-secondary">Черновик</button><button type="submit" class="btn btn-primary">На утверждение</button></div></form></div></div></div>
</div></div></div>
</body></html>'''
...
<div class="d-flex"><span class="navbar-text me-3">{{ current_user.full_name }}</span><a href="/logout" class="btn btn-outline-light btn-sm">Выход</a></div></div></nav>
<div class="container-fluid"><div class="row"><div class="col-md-2 sidebar p-3">
<div class="text-center mb-4"><div class="bg-primary text-white rounded-circle d-flex align-items-center justify-content-center mx-auto mb-2" style="width:60px;height:60px">{{ current_user.full_name[0] }}</div><h6>{{ current_user.full_name }}</h6><small class="text-muted">{{ current_user.role }}</small></div>
<nav class="nav flex-column"><a class="nav-link" href="/">Рабочий стол</a><a class="nav-link active" href="/orders">Распоряжения</a></nav>
</div><div class="col-md-10 p-4">
<a href="/orders" class="btn btn-secondary btn-sm mb-3">← Назад</a>
<div class="card p-4"><h3>{{ order.title }}</h3><div class="row mt-3"><div class="col-md-3"><small>Автор</small><br><strong>{{ order.creator_name }}</strong></div>
...
<div class="col-md-3"><small>Создано</small><br><strong>{{ order.created_at[:10] }}</strong></div></div>
<hr><div class="bg-light p-3 rounded">{{ order.content }}</div></div>
<div class="card p-4 mt-3"><h5>Действия</h5>
...
<form method="POST" action="/orders/{{ order.id }}/status"><input type="hidden" name="status" value="Отклонено"><textarea name="comment" class="form-control mb-2" placeholder="Причина"></textarea><button class="btn btn-danger w-100">✗ Отклонить</button></form>{% endif %}
...
<form method="POST" action="/orders/{{ order.id }}/status"><input type="hidden" name="status" value="В работе"><button class="btn btn-primary w-100">Взять в работу</button></form>{% endif %}
{% if current_user.role == 'executor' and order.status == 'В работе' and order.assigned_executor_id == current_user.uid %}
<form method="POST" action="/orders/{{ order.id }}/submit" enctype="multipart/form-data"><textarea name="result_content" class="form-control mb-2" rows="4" placeholder="Результат" required></textarea><input type="file" name="result_file" class="form-control mb-2"><button class="btn btn-success w-100">Сдать работу</button></form>{% endif %}
...
<form method="POST" action="/orders/{{ order.id }}/status"><input type="hidden" name="status" value="На доработке"><textarea name="comment" class="form-control mb-2" placeholder="Причина"></textarea><button class="btn btn-warning w-100">На доработку</button></form>{% endif %}
{% if current_user.role == 'head_central' and order.status == 'Подтверждено' %}
<form method="POST" action="/orders/{{ order.id }}/status"><input type="hidden" name="status" value="Закрыто"><button class="btn btn-success w-100">Закрыть</button></form>{% endif %}
</div>
</div></div></div>
</body></html>'''
...
<div class="card p-3"><div class="d-flex justify-content-between"><h5>Пользователи</h5><div><a href="/admin/export/users" class="btn btn-success btn-sm">Excel</a><button class="btn btn-primary btn-sm ms-2" data-bs-toggle="modal" data-bs-target="#addUserModal">+ Добавить</button></div></div>
<table class="table"><thead><tr><th>ФИО</th><th>Логин</th><th>Роль</th><th>Email</th><th>Отдел</th></tr></thead>
<tbody>{% for u in users %}<tr><td>{{ u.full_name }}</td><td>{{ u.username }}</td><td>{{ u.role }}</td><td>{{ u.email }}</td><td>{{ u.department_id or '-' }}</td></tr>{% endfor %}</tbody></table></div>
...
<html lang="ru">
<head><meta charset="UTF-8"><title>Статистика - ЭДО ЛДПР</title>
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css" rel="stylesheet">
<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.3/font/bootstrap-icons.css">
<style>.navbar{background:#003399}.sidebar{background:white;min-height:100vh}</style>
</head>
...
<h2>Статистика</h2>
<div class="row"><div class="col-md-3"><div class="card p-3 text-center"><h3>{{ user_stats.total }}</h3><small>Пользователей</small></div></div>
<div class="col-md-3"><div class="card p-3 text-center"><h3>{{ dept_stats.total }}</h3><small>Отделов</small></div></div>
<div class="col-md-3"><div class="card p-3 text-center"><h3>{{ order_stats.total }}</h3><small>Распоряжений</small></div></div>
<div class="col-md-3"><div class="card p-3 text-center"><h3>{{ completed_orders }}</h3><small>Выполнено</small></div></div></div>
</div></div></div>
</body></html>'''
...
def create_order():
    ...
    if 'order_file' in request.files:
        f = request.files['order_file']
        if f and f.filename:
            order_file = save_uploaded_file(f, order_id, session['user_id'])
    OrderModel.create(order_id, title, content, request.form.get('priority', 'Нормальный'), status, session['user_id'], user['full_name'], request.form.get('deadline') or None, order_file)
...
def submit_order_result(order_id):
    ...
    if 'result_file' in request.files:
        ...
        if f and f.filename:
            result_file = save_uploaded_file(f, order_id, session['user_id'])
...
def admin_export_orders():
    ...
    excel = export_orders_to_excel(orders)
    if excel:
...
def admin_export_users():
    users = UserModel.get_all()
    excel = export_users_to_excel(users)
    if excel:
        return send_file(excel, mimetype='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
