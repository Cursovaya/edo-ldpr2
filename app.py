...
Все функции: распоряжения, отделы, пользователи, статусы, история
Полный интерфейс со всеми страницами
"""
...
<div class="row mb-4">
    <div class="col-md-3 mb-3">
        <div class="card stat-card p-3">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <small class="text-muted text-uppercase fw-bold">Мои распоряжения</small>
                    <h2 class="fw-bold mb-0 mt-2">{{ stats.total }}</h2>
                </div>
    ...
    <div class="col-md-3 mb-3">
        <div class="card stat-card p-3" style="border-left-color: #f59e0b;">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <small class="text-muted text-uppercase fw-bold">На утверждении</small>
                    <h2 class="fw-bold mb-0 mt-2">{{ stats.pending }}</h2>
                </div>
    ...
    <div class="col-md-3 mb-3">
        <div class="card stat-card p-3" style="border-left-color: #10b981;">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <small class="text-muted text-uppercase fw-bold">Утверждено</small>
                    <h2 class="fw-bold mb-0 mt-2">{{ stats.approved }}</h2>
                </div>
    ...
    <div class="col-md-3 mb-3">
        <div class="card stat-card p-3" style="border-left-color: #6366f1;">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <small class="text-muted text-uppercase fw-bold">В работе</small>
                    <h2 class="fw-bold mb-0 mt-2">{{ stats.in_work }}</h2>
                </div>
...
{% block title %}Реестр распоряжений{% endblock %}
{% block content %}
<div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <div class="timeline">
                ...
                <div class="d-flex mb-3">
                    ...
                    <div>
                        ...
                        <p class="text-muted mb-0 small">{{ item.details or '' }}</p>
                        <small class="text-muted">{{ item.user_name }} · {{ item.created_at[:16] if item.created_at else '' }}</small>
                    </div>
                </div>
                {% endfor %}
            </div>
...
{% block title %}Управление отделом{% endblock %}
{% block content %}
        ...
        <div class="card p-4">
            ...
            <div class="table-responsive">
                <table class="table table-hover">
                    <thead class="table-light">
                        <tr>
                            <th>ФИО</th><th>Логин</th><th>Роль</th><th>Email</th><th>Отдел</th><th>Действия</th>
                        </tr>
                    </thead>
                    <tbody>
                        {% for u in users %}
                        <tr>
                            ...
                            <td>{{ u.email }}</d>
                            <td>
                                {% for d in departments %}{% if d.id == u.department_id %}{{ d.name }}{% endif %}{% endfor %}
                            </d>
                            <td>
                                <button class="btn btn-sm btn-outline-danger" onclick="if(confirm('Удалить?')) location.href='/admin/users/{{ u.uid }}/delete'">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <button class="btn btn-sm btn-outline-danger" onclick="if(confirm('Удалить отдел?')) location.href='/admin/departments/{{ d.id }}/delete'">
                                    <i class="bi bi-trash"></i>
                                </button>
                            </d>
                        </tr>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <form method="POST" action="/admin/users/create">
                <div class="modal-body">
                    <div class="row">
                        <div class="col-md-6 mb-3"><label>ФИО</label><input type="text" name="full_name" class="form-control" required></div>
                        <div class="col-md-6 mb-3"><label>Логин</label><input type="text" name="username" class="form-control" required></div>
                        <div class="col-md-6 mb-3"><label>Email</label><input type="email" name="email" class="form-control" required></div>
                        <div class="col-md-6 mb-3"><label>Пароль</label><input type="password" name="password" class="form-control" required></div>
